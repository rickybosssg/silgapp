    ...
    switch (toolName) {
      ...
      case 'consulter_tarifs': {
        return JSON.stringify({
          ...
          commission_silga: Number.isFinite(Number(ctx.tarifs?.commission_pct))
            ? `${Number(ctx.tarifs.commission_pct)}%`
            : null,
          gain_livreur: Number.isFinite(Number(ctx.tarifs?.commission_pct))
            ? `${100 - Number(ctx.tarifs.commission_pct)}%`
            : null,
        });
      }
