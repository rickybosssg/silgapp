...
async function handleDeblocage(base44, body) {
  ...
  await base44.asServiceRole.entities.Livreur.update(livreur_id, {
    ...
    admin_statut_log: 'Déblocage encours par administrateur',
  ...
  if (livreur.user_email) {
    ...
    await base44.asServiceRole.entities.Notification.create({
      ...
      message: reset_complet === false
        ? `Votre encours a été réduit de ${reduction.toLocaleString()} ${devise}. Nouvel encours : ${nouvelEncours.toLocaleString()} ${devise}.`
  ...
  console.log(`[ENCOURS] DÉBLOCAGE: Livreur ${livreur_id} par ${user.email} — ${encoursAvant} → ${nouvelEncours}`);
  return Response.json({ success: true, encours_avant: encoursAvant, encours_apres: nouvelEncours });
}
...
async function handleAjustement(base44, body) {
  ...
  const seraBloque = nouvel_encours < seuil;

  await base44.asServiceRole.entities.Livreur.update(livreur_id, {
    encours: nouvel_encours,
    bloque_encours: !seraBloque,
    encours_bloque_at: seraBloque ? now : null,
...
async function handleGetBloques(base44, body) {
  ...
  const filter = { bloque_encours: true, type_livreur: 'externe' };
