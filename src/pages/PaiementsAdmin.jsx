...
import { ArrowLeft, CheckCircle2, XCircle, Clock, Image as ImageIcon, Phone, X } from "lucide-react";
...
function PaiementCard({ paiement, onTraiter, onRefuser, isPending }) {
  ...
  const isPending_ = paiement.statut === "en_attente";

  return (
    <div className={`bg-white rounded-2xl border p-4 shadow-sm ${isPending_ ? "border-orange-200" : "border-gray-100"}`}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${USER_TYPE_COLOR[paiement.user_type] || "bg-gray-100 text-gray-700"}`}>
        ...
        {paiement.date_envoi && (
          <span className="text-xs text-gray-400">{format(new Date(paiement.date_envoi), "dd/MM/yyyy 'à' HH:mm", { locale: fr })}</span>
        )}
      </div>
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-bold text-gray-900 truncate">{paiement.user_nom || "Inconnu"}</p>
          {paiement.user_telephone && (
            <p className="text-xs text-gray-400 flex items-center gap-1"><Phone className="w-3 h-3" />{paiement.user_telephone}</p>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-gray-50 rounded-xl p-2.5 text-center">
          ...
          <p className="font-bold text-gray-800">{(paiement.montant_du || 0).toLocaleString()} F</p>
        </div>
      ...
      <div className="flex items-center gap-2 mb-3 text-xs text-gray-500">
        <span>Dépôt : <strong className="text-gray-700">{paiement.numero_depot || "+226 66 92 51 90"}</strong></span>
      </div>
      ...
      {isPending_ && (
        <div className="flex gap-2">
          <Button className="flex-1 bg-green-600 hover:bg-green-700" disabled={isPending} onClick={() => onTraiter(paiement)}>
            <CheckCircle2 className="w-4 h-4 mr-1" />Traiter
          </Button>
          <Button variant="destructive" className="flex-1" disabled={isPending} onClick={() => onRefuser(paiement)}>
      ...
      {showProof && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => setShowProof(false)}>
          <div className="relative max-w-md w-full">
            <button onClick={() => setShowProof(false)} className="absolute -top-10 right-0 text-white"><X className="w-6 h-6" /></button>
  ...
  return (
    <div className="px-4 py-4 max-w-2xl mx-auto space-y-4">
      <div className="flex items-center gap-3">
        <Link to="/"><Button variant="ghost" size="sm"><ArrowLeft className="w-5 h-5" /></Button></Link>
        <div>
          <h1 className="text-lg font-black text-gray-900">Paiements SILGAPP</h1>
          <p className="text-xs text-gray-500">Preuves de dépôt reçues</p>
        </div>
      </div>
      ...
      {nbEnAttente > 0 && (
        <div className="bg-gradient-to-br from-orange-500 to-amber-500 rounded-2xl p-4 text-white flex items-center justify-between">
          <div>
            ...
            <p className="text-3xl font-black">{nbEnAttente}<span className="text-sm font-normal ml-2">paiement{nbEnAttente > 1 ? "s" : ""}</span></p>
          </div>
          <div className="text-right">
            <p className="text-xs opacity-80">Total</p>
            <p className="text-2xl font-black">{totalEnAttente.toLocaleString()}<span className="text-sm font-normal ml-1">F</span></p>
          </div>
        </div>
      )}
