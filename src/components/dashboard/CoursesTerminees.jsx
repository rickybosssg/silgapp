    ...
    <div className="flex items-center justify-between gap-3 py-2.5 border-b border-gray-100 last:border-0">
      <div className="flex-1 min-w-0 space-y-1.5">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="font-bold text-sm text-foreground">{course.client_nom || course.client_telephone}</span>
          <CourseStatusBadge statut={course.statut} />
          {course.livreur_nom && (
            <span className="text-[10px] text-gray-500">→ {course.livreur_nom}</span>
          )}
        </div>
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        {course.prix_final > 0 && (
          <span className="text-xs font-bold text-emerald-600">{course.prix_final.toLocaleString()} F</span>
        )}
  ...
  return (
    <Card className="p-0 overflow-hidden bg-card border-border">
      <div className="px-4 pt-4 pb-3 border-b border-gray-100 flex items-center justify-between">
        ...
        <p className="text-xs text-gray-500">Livrées & annulées</p>
      </div>
      <div className="px-4 pb-2">
        {courses.length === 0 ? (
          <p className="text-center py-6 text-gray-500 text-sm">Aucune course terminée aujourd'hui</p>
        ) : (
          <>
            {affichees.map(c => (
            ...
            {courses.length > 5 && (
              <button
                ...
                className="w-full py-2 text-xs text-emerald-600 flex items-center justify-center gap-1 hover:underline"
              >
