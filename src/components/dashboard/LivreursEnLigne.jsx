  ...
  return (
    <div className="p-4 bg-card rounded-2xl border border-border">
          ...
          {livreurs.length > 0 && (
            <>
              <span className="flex items-center gap-1.5 text-xs">
                ...
                <span className="font-black text-green-600">{disponibles.length}</span>
              </span>
              <span className="flex items-center gap-1.5 text-xs">
                <span className="w-2 h-2 rounded-full bg-orange-400 inline-block" />
                <span className="text-gray-600">En course :</span>
                <span className="font-black text-orange-600">{enCourse.length}</span>
              </span>
            </>
          )}
        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          <div className="rounded-lg bg-green-50 border border-green-200 px-2.5 py-1.5">
            <div className="flex items-center justify-between mb-0.5">
              <span className="text-[9px] font-bold text-green-600 uppercase">Taux dispo</span>
              <span className="text-xs font-black text-green-600">{tauxDispo}%</span>
            </div>
          ...
          <div className="rounded-lg bg-blue-50 border border-blue-200 px-2.5 py-1.5">
            <div className="flex items-center justify-between mb-0.5">
              <span className="text-[9px] font-bold text-blue-600 uppercase">GPS actif</span>
              <span className="text-xs font-black text-blue-600">{avecGPS.length}/{livreurs.length}</span>
            </div>
      ...
      {open && (
        <div className="mt-3">
          {livreurs.length === 0 ? (
            <p className="text-xs text-gray-500 text-center py-3">Aucun livreur en ligne</p>
          ) : (
            <>
                  ...
                  return (
                    ...
                    >
                      {l.photo_url ? (
                        ...
                        <span className="w-5 h-5 rounded-full bg-gray-100 flex items-center justify-center flex-shrink-0 text-[9px] font-bold text-gray-600">
              ...
              <div className="mt-3 text-right">
                <Link to="/livreurs" className="text-xs text-emerald-600 hover:underline">Voir tous les livreurs →</Link>
              </div>
            </>
          )}
        </div>
