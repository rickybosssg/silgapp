...
function GeneralChatWindow({ conversationId, myType, myId, myName, onBack }) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0A1F3D]">
      {/* Header */}
      <div className="flex items-center gap-2 p-3 bg-[#071730] text-white">
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto p-3 bg-[#0A1F3D]">
        {messages.length === 0 && (
      ...
      <div className="p-2.5 bg-[#0C2350] border-t border-white/10 flex items-end gap-1.5 safe-area-bottom shadow-[0_-8px_24px_rgba(0,0,0,0.2)]">
        <AudioRecorder
        ...
        <textarea
          ...
          disabled={sending}
          rows={2}
          className="flex-1 min-h-14 max-h-32 min-w-0 resize-none rounded-2xl border border-white/10 bg-[#0A1F3D] px-4 py-3 text-[15px] font-medium leading-5 text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-[#00a86b]/40 focus:border-[#00a86b]/40"
        />
        <Button
          onClick={handleSend}
