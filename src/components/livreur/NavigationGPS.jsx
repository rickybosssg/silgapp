...
export default function NavigationGPS({
  ...
  // Cas destination inconnue ET pas de GPS destinataire
  if (destinationInconnue && !canNavigate) {
    return (
  ...
  // Navigation disponible (coordonnées fixes ou GPS live destinataire)
  return (
    <div className="bg-green-50 border border-green-200 rounded-2xl p-4 space-y-3">
      {/* Bandeau destinataire en ligne */}
      <DestStatut />

      {/* Badge "GPS live" */}
      {usesLiveGps && (
        <div className="flex items-center gap-1.5 bg-green-200/50 rounded-lg px-2 py-1">
          <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
          <p className="text-[10px] font-bold text-green-700">
            Position GPS temps réel du destinataire
          </p>
        </div>
      )}
      ...
      {/* Bouton "Voir en direct" sur carte */}
      {usesLiveGps && (
        <button
      ...
      {showMiniMap && effectiveLat && effectiveLng && (
        <MiniMap
