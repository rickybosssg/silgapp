...
async function invokeCourseBackend(functionName, payload) {
  const response = await base44.functions.invoke(functionName, payload);
  const data = response?.data ?? response;
  if (data?.success === false || data?.error) {
    throw new Error(data?.error || `Erreur ${functionName}`);
  }
  return data;
}
...
export default function CourseActiveCard({ course, onColisRecupere, onColisLivre, onClientAnnule, onMettrePause, isPending, isExterne = false, livreurLat, livreurLng, livreurId, livreurNom }) {
  ...
  useEffect(() => {
    ...
    const autoSetPrice = async () => {
      try {
        await invokeCourseBackend("finaliserLivraisonLivreur", {
          course_id: course.id,
  ...
  const handleConfirmerLivraison = () => {
    ...
    if (isDeplacement) {
      ...
      // Le montant reste local jusqu'au bouton final afin d'éviter toute écriture
      // sensible avant la confirmation explicite du livreur.
  ...
  const handleClientContacte = async () => {
    ...
    await invokeCourseBackend("transitionStatutLivreur", {
      course_id: course.id,
          ...
          >
            ...
            <div className="text-center px-6 pt-6 pb-3 flex-shrink-0">
              ...
              <p className="text-sm text-gray-500 mt-1">
                {isDeplacement
                  ? "Pourquoi souhaitez-vous annuler ce déplacement ?"
              ...
              <div className="space-y-3 pt-1">
                ...
                {!isPrisEnCharge && !isArrivee && !isTermine && (
                  <button
                    ...
                    onClick={() => setShowAnnulerCourse(true)}
                    disabled={isPending}
                  >
                    <X className="w-4 h-4" /> ANNULER LA COURSE
                  </button>
                )}
              </div>
            );
