...
import {
  Users, Phone, Clock, CheckCircle2, RefreshCw, Radio, Zap, XCircle, UserCheck,
...
export default function ProposedLivreursListV2({ course }) {
  ...
  useEffect(() => {
    ...
    base44.entities.AppConfig.filter({ cle: "DISPATCH_V2_ENABLED" })
      .then((configs) => {
        if (mounted) setV2Enabled(configs?.[0]?.valeur === "true");
      })
      .catch(() => {});
    ...
    const fetchEligible = async () => {
      try {
        const livreurs = await base44.entities.Livreur.filter(
          { country_code: course.country_code, statut: "disponible", actif: true, valide: "valide" },
          "-created_date", 500
        );
        if (!mounted) return;
