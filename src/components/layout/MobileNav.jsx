    ...
    <>
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 border-b border-white/8 z-40 flex items-center justify-between px-4 shadow-sm safe-area-top" style={{ minHeight: '3.5rem', backgroundColor: '#16191d' }}>
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[9px] text-white/40 leading-tight">SILGAPP Livraison</p>
          </div>
        </div>
      ...
      {showMenu && (
        <div className="lg:hidden fixed inset-0 z-50" onClick={() => setShowMenu(false)}>
          ...
          <div
            className="absolute right-0 top-0 bottom-0 w-72 flex flex-col shadow-2xl border-l border-white/8"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Menu header */}
            <div className="h-14 flex items-center justify-between px-4 border-b border-white/8 flex-shrink-0">
              <div>
                {user && (
                  <div>
            ...
            {showCountryPicker && (
              <div className="px-3 py-2 border-b border-white/8">
                ...
                {countryOpen && (
                  <div className="mt-1 border border-white/8 rounded-xl shadow-lg max-h-56 overflow-y-auto" style={{ backgroundColor: '#1f2429' }}>
                    {PAYS_SILGAPP.map((p) => (
                      <button
                        key={p.code}
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 border-t border-white/8 z-40 safe-area-bottom" style={{ backgroundColor: '#16191d' }}>
